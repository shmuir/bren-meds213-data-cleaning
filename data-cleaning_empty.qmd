...
```{r}
range(snowsurvey_fixed$Snow_cover, na.rm = TRUE)
...
```{r}
write_csv(snowsurvey_fixed, "data/processed/snow_cover.csv")
```
...
```{r}
species_presence <- species_fixed %>%
...
```{r}
write_csv(species_presence, "data/species_presence.csv")
```
